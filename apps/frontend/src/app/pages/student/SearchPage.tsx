...
const defaultParams: Params = {
    ...
    levels: [],
    language: [],
    organization: queryParams.has('organization') ? queryParams.get('organization')! : undefined,
    price: {
        start: undefined,
        end: undefined,
    },
...
function ResponsiveDrawer(props: any) {
    ...
    const subjectsList = ['Business', 'Computer Science', 'Data Science', 'Health', 'Information Technology', 'Math and Logic', 'Physical Science and Engineering'];
    ...
    const drawer = (
        <List sx={{ mx: { xs: 2, sm: 0 }, mr: { sm: 2 }, mt: { xs: 3 } }}>
            ...
            <FormControl component="fieldset" sx={{ mt: 4, display: 'flex' }}>
                ...
                <FormGroup>
                    {
                        subjectsList.filter((text, index, arr) => showMore.subjects || index < 4).map((text) => (
                            <FormControlLabel control={<Checkbox onChange={() => setParams((prevState: Params) => ({ ...prevState, subjectArea: updateArray(prevState.subjectArea, text) }))} />} label={text} />
                        ))
                    }
                </FormGroup>
                <Button variant="text" onClick={() => setShowMore(prevState => ({ ...prevState, subjects: !prevState.subjects }))}>
                    {subjectsList.length < 4 ? '' : showMore.subjects ? 'Show less' : 'Show more'}
                </Button>
            </FormControl>
            ...
            <FormControl component="fieldset" sx={{ mt: 4, display: 'flex' }}>
                ...
                <FormGroup>
                    {
                        levelsList.filter((text, index, arr) => showMore.levels || index < 4).map((text) => (
                            <FormControlLabel control={<Checkbox onChange={() => setParams((prevState: Params) => ({ ...prevState, levels: updateArray(prevState.levels, text) }))} />} label={text} />
                        ))
                    }
                </FormGroup>
                <Button variant="text" onClick={() => setShowMore(prevState => ({ ...prevState, levels: !prevState.levels }))}>
                    {levelsList.length < 4 ? '' : showMore.levels ? 'Show less' : 'Show more'}
                </Button>
            </FormControl>
            ...
            <FormControl component="fieldset" sx={{ mt: 4, display: 'flex' }}>
                ...
                <FormGroup>
                    {
                        languageList.filter((text, index, arr) => showMore.language || index < 4).map((text) => (
                            <FormControlLabel control={<Checkbox onChange={() => setParams((prevState: Params) => ({ ...prevState, language: updateArray(prevState.language, text) }))} />} label={text} />
                        ))}
                </FormGroup>
                <Button variant="text" onClick={() => setShowMore(prevState => ({ ...prevState, language: !prevState.language }))}>
                    {languageList.length < 4 ? '' : showMore.language ? 'Show less' : 'Show more'}
                </Button>
            </FormControl>
                ...
                <FormGroup sx={{ px: { xs: 3, sm: 2 } }}>
                    <Slider
                        ...
                        onChange={(event: Event, newValue: number | number[]) => {
                            ...
                            Array.isArray(newValue) && setParams((current: Params) => ({ ...current, price: { ...current.price, start: newValue[0], end: newValue[1] } }))
                        }}
...
export default function SearchPage() {
    ...
    React.useEffect(() => {
        ...
        setParams((prevState: Params) => ({
            ...
            organization: queryParams.has('organization') ? queryParams.get('organization')! : '',
        }))
                ...
                <Box sx={{ display: 'flex', margin: '0 auto' }}>
                    ...
                    <Box sx={{ flex: 1, mr: { sm: 0, md: 5, lg: 10 }, }}>
                        <Box sx={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'center', mt: 0, mb: 4, width: { sm: '70%', md: '50%' } }}>
                            ...
                            <TextField label="Search Courses" variant="standard" fullWidth color="primary" sx={{ mr: 2 }} defaultValue={params.searchQuery} value={params.searchQuery}
                        ...
                        <Grid container spacing={2} >
                            {(isLoading || isError || !courses || isRefetchError || isRefetching)
                                ? [...Array(5).keys()].map((_) => (
